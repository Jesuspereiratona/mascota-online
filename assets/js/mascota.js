class Pet {
    ...
    eat() {
        ...
        console.log(this.status) //Tiraba error ya que faltaba en console.log
    }

    play() {
        if (!this.#isAlive) {        // Descontaba antes del if... no verificaba y faltaba # (2 en 1)
            this.#satiety = 0
            this.#energy = 0
        ...
        console.log(this.status) //mismo error previo, funciona es con console.log 
    }
    ...
    get energy()  { return this.#energy  } //ambos get para actualizacion de las barras...estaban faltando 
    get satiety() { return this.#satiety }
}
...
// UI - User Interface _ UX User Experience (errores, faltaban los get, sino quedan nulos)
...
btnComer.addEventListener('click', () => {
    const mascotaImg = document.getElementById('mascota-img')
    pet5.eat()                        // mascota usada es pet5
    mascotaImg.src = "./assets/img/pet-eating.png"
    actualizarUI(pet5)                // agregar pet5 dentro de la avtualizacion ui 
})

btnJugar.addEventListener('click', () => {
    const mascotaImg = document.getElementById('mascota-img')
    pet5.play()                       //   mascota -> pet5
